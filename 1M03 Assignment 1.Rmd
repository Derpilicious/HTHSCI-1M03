...
bruh kenny i still dk why we need to use unique and levels because that shit is only for a factor but x2 is originally a factor so why tf we need to convert a numeric factor into a numeric factor bruh clearly he means to convert a numeric factor into a numeric vector and we just need to use as.character() for that shit so yeah bruh idk i feel like the hint is either misleading or i am severely misunderstadning this question lmfao man whatever we just lose 0.6% not even a big deal they drop the worst one too so we just do better next two
