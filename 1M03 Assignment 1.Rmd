...
author: "Roy Luo 400474680, Kenny Zhao ADD YOUR FUCKING STUDENT NUMBER OMG YOU DUMB SHIT YOURE ACTUALLY INFECTED"
date: "2023-01-30"
output: pdf_document
---
...
```
Uses the **table()** function to see the count of how many times each level appears in **x1**. Then uses which.min() to find the index of the level that appears the least amount of times. Finally uses names() to display what the actual level is that has appeared the least amount of times in **x1**.
...
str(x2)
```
Assigns **x2** to the numeric version of itself using the **as.numeric()** function. Displays the result using str().
...
library(dslabs)
```
Loads the **dslabs** library using the **library()** function.
...
```
As the **dslabs** library is already loaded from question 3a, there is no need to load it again using the **library()** function. Displays the **greenhouse_gases** data using the **str()** function.
