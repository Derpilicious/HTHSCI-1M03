...
```
yo keep doing this until theres no more outliers or weird numbers or whatever just use summary() and then the command above to turn it into NA
```{r}
tib %>%
  .$column with outliers %>%
  summary()
...
```{r}
tib %>% count(character column) %>% kable()
```
It seems like character column does not need to be fixed.
```{r}
...
tib %<>% mutate(new name = if_else(other character column %in% vm, "whatever you want them all to be", other character column))
```
...
```{r}
tib %$% table(other character column, new name)
```
**Convert character vectors into a factor.**
```{r}
tib %<>% mutate(char to factor = as.factor(char to factor)) #yo just do this for every charavter column ya wanna turn into a factor jit
```
...
Let's drop all NA values from the tibble.
```{r}
tib %<>% drop_na()
```
Now let's check every column to make sure there are no more NA values.
```{r}
tib %>% pull(everything()) %>% is.na() %>% any()
```
